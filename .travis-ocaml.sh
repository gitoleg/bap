...
full_apt_version () {
  ...
  case "${version}" in
      ...
      *) R=`apt-cache show "$package" \
             | sed -n "s/^Version: \(${version}\)/\1/p" \
             | head -1`;
         if [ -n "$R" ] ; then echo -n "${package}=${R}"
         else echo -n "${package}"
         fi ;;
  esac
}
...
OCAML_VERSION=${OCAML_VERSION:-4.04}
...
install_on_linux () {
  case "$OCAML_VERSION,$OPAM_VERSION" in
    ...
    4.03,1.2.2)
       OCAML_VERSION=4.03; OPAM_SWITCH="4.03.0" ;;
       # ppa=avsm/ocaml42+opam12 ;;
    4.04,1.2.2)
        OCAML_VERSION=4.04; OPAM_SWITCH="4.04.0" ;;
        # ppa=avsm/ocaml42+opam12 ;;
