...
let supported_api (module Abi : abi) {C.Type.Proto.return; args} =
  let word = Arch.addr_size (`sparcv9 :> arch) |> Size.in_bits in
...
let main proj = match Project.arch proj with
  | `sparcv9 ->
    info "using sparc v9 ABI";
    let abi = C.Abi.{
        insert_args = dispatch (module Abi64);
        apply_attrs = fun _ -> ident
      } in
    let api = C.Abi.create_api_processor Abi64.size abi in
    Bap_api.process api;
    let prog = Project.program proj in
    Project.set (Project.with_program proj prog) Bap_abi.name Abi64.name
  | _ -> proj
